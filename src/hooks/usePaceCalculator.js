...
export const usePaceCalculator = () => {
        ...
        setInputs(prev => {
            ...
            if (category === 'time' && field) {
                ...
                if (trimmedValue && trimmedValue !== '0') {
                    if (field === 'hours' && !prev.time.minutes)
                        newInputs.time.minutes = '00';
                    if (field === 'hours' && !prev.time.seconds)
                        newInputs.time.seconds = '00';
                    if (field === 'minutes' && !prev.time.seconds)
                        newInputs.time.seconds = '00';
                }
                ...
                // Reset lower empty fields to '00' when entering a value
                if (trimmedValue && trimmedValue !== '0') {
                    if (field === 'minutes' && !prev.pace.seconds)
                        newInputs.pace.seconds = '00';
                }
                // Auto-format single digits with leading zeros when larger values exist
    ...
    const isCalculatedField = useCallback((type) => {
    ...
    const calculateActiveField = useCallback(() => {
        try {
            if (activeCalculation === 'pace') {
                ...
                const totalTimeSeconds = timeUtils.timeToSeconds(inputs.time.hours, inputs.time.minutes, inputs.time.seconds);
                ...
                const paceSeconds = timeUtils.paceToSeconds(inputs.pace.minutes, inputs.pace.seconds);
                const totalTimeSeconds = timeUtils.timeToSeconds(inputs.time.hours, inputs.time.minutes, inputs.time.seconds);
