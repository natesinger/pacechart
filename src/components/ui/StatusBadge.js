...
export const StatusBadge = ({ type, variant = 'default', className }) => {
    const config = badgeConfig[type];
    const Icon = config.icon;
    return (_jsxs("div", { className: cn('flex items-center gap-2 text-xs font-semibold px-3 py-2 rounded-lg border shadow-lg transition-all duration-200', config.colors[variant], className), children: [_jsx(Icon, { className: "w-3 h-3" }), config.text] }));
};
